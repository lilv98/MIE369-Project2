...
//
// - Here, single equality predicates are kept separate under the
//   "EQUAL" key and bought in at each resolution step for 
//   demodulation.
//
// - Demodulation rule:
//
//   t1 = t3;   A(... t2 ...)     s = unify(t1,t2)
//   ------------------------
//        A(... t3_s ...)         [Note: substitution s is local!]
//////////////////////////////////////////////////////////////////////
...
public class DemodClauseKb
    ...
    public static final boolean PRINT_DEMOD            = false;
    ...
    public static final boolean GEN_GRAPH              = false;
    ...
    public HashSet    _hsEquality;    // Set of equality predicates
    public List       _lElimOrdering; // Ordering of predicates (as Strings: name_arity)
    public Graph      _graphClauses;  // A graph of connectivity b/w predicates due to clauses
    public BindingSet _bs;            // Store query results
    public boolean    ALL_PROOFS;     // Retrieve one proof or all proofs (for getting query bindings)
    public int        MAX_RES_PER_ELIM; // In case of infinite resolvents
    public ArrayList  _alQueryBindings;

    public DemodClauseKb() {
	this(100);
    } 

    public DemodClauseKb(int max_res) {
	_hmPred2Clause   = new HashMap();
	_hmPred2Name     = new HashMap();
	_hmPred2Arity    = new HashMap();
	_hmProofs        = null;
	_hsEquality      = new HashSet();
	_lElimOrdering   = null;
	_graphClauses    = null;
	_bs              = null;
	_alQueryBindings = null;
	ALL_PROOFS       = false;
	MAX_RES_PER_ELIM = max_res;
    }
	...
	addFormula(new_clauses, n.convertNNF(true));
	...
	FOPC.Node imp_node = new FOPC.ConnNode(FOPC.ConnNode.IMPLY, 
					       query, query_node);
	...
	free_vars.remove("EQUALS_0");
	Set factors = new HashSet();
	while (i.hasNext()) {
	    Map.Entry me = (Map.Entry)i.next();
	    String key = (String)me.getKey();
	    if (key.equalsIgnoreCase("EQUALS_0")) {
		continue;
	    }
		...
		if (n instanceof FOPC.ConnNode) {
		    ...
		    System.out.println("Error in pos clause list: " + n.toFOLString());
		    System.exit(1);
		}
	    }
		...
		if (n instanceof FOPC.ConnNode) {
		    ...
		    System.out.println("Error in neg clause list: " + n.toFOLString());
		    System.exit(1);
		}
	    }
	}
	...
	if (GEN_GRAPH) {
	    _graphClauses.genDotFile("tp.dot");
	}
    ...
    // Returns true if kb inconsistent (will modify clauses so should be a copy!)
	...
	// First determine an elimination order (important if new predicates added)
	if (_lElimOrdering == null || !clauses.keySet().equals(new HashSet(_lElimOrdering))) {
	...
	// of clauses below.
	ClauseList cl = (ClauseList)getClauseList(clauses, pred, false).clone();
	if (cl == null) {
	...
	// Now add all equality axioms
	ClauseList eq = getClauseList(clauses, "EQUALS_0", false);
	//System.out.println("EQUALS: " + eq);
	ArrayList equality_cl = ((eq == null) ? new ArrayList() : eq._alPosClauses);
	cl._alPosClauses.addAll(equality_cl);
	cl._alNegClauses.addAll(equality_cl);
	...
	while (j.hasNext()) {
		...
		if (((FOPC.TNode)n)._bValue) {
			...
			return true; // Refutation found, don't need all proofs!
		    }
		}
	    } else if (n instanceof FOPC.ConnNode) {
		addToClauseLists(clauses, n);
	    } else {
		System.out.println("Illegal FOPC.Node durign res: " + n.toFOLString());
		System.exit(1);
	    }
	}
	...
	do {
		...
		for (int neg_it = 0; neg_it < cl._alNegClauses.size(); neg_it++) {
		    FOPC.ConnNode n2 = (FOPC.ConnNode)cl._alNegClauses.get(neg_it);
		    FOPC.NodePair p = new FOPC.NodePair(n1, n2);

		    // Have we performed this resolution already?  **This is key to
		    // efficiency because it compares the actual standardized clauses
		    // to see if they've been resolved against each other before.
		    if (!res_set.contains(p)) {
			res_set.add(p);
			
			// Determine all possible resolutions of n1, n2 over pred
			int sz1 = n1._alSubNodes.size();
			for (int pos1 = 0; pos1 < sz1; pos1++) {
			    FOPC.PNode p1 = (FOPC.PNode)n1._alSubNodes.get(pos1);

			    // TODO: Resolve if nodes are disjoint

			    if (!(sz1 == 1 && p1._nPredID == FOPC.PNode.EQUALS && 
				  !p1._bIsNegated) &&
				(p1._nArity != arity || p1._bIsNegated ||
				 !(""+p1._sPredName).equals(""+predname))) {
				//System.out.println("Skipping: " + p1.toFOLString());
				continue;
			    }
			    int sz2 = n2._alSubNodes.size();
			    for (int pos2 = 0; pos2 < sz2; pos2++) {
				FOPC.PNode p2 = (FOPC.PNode)n2._alSubNodes.get(pos2);

				// TODO: Resolve if nodes are disjoint

				if (!(sz2 == 1 && p2._nPredID == FOPC.PNode.EQUALS && 
				      !p2._bIsNegated) &&
				    (p2._nArity != arity || !p2._bIsNegated ||
				     !(""+p2._sPredName).equals(""+predname))) {
				    //System.out.println("Skipping: " + p2.toFOLString());
				    continue;
				}

				// Do not resolve *two* single equality statements
				if ((sz1 == 1 && p1._nPredID == FOPC.PNode.EQUALS && 
				     !p1._bIsNegated) &&
				    (sz2 == 1 && p2._nPredID == FOPC.PNode.EQUALS && 
				     !p2._bIsNegated)) {
				    continue;
				}
				...
				if ((sz1 == 1 && p1._nPredID == FOPC.PNode.EQUALS && 
				    ...
				    // Demodulate
				    results = demodClauses(n1, pos1, n2, pos2);		

				} else {		

				    // Resolve
				    results = new ArrayList();
				    FOPC.Node res = resolveClauses(n1, pos1, n2, pos2);
				    if (res != null) {
					results.add(res);
				    }
				}
				...
				// Go through all resolvents
				Iterator res_i = results.iterator();
				while (res_i.hasNext()) {
				    FOPC.Node res = (FOPC.Node)res_i.next();
				    
				    // Keep track of proof
				    if (MAINTAIN_PROOFS &&
					(!(res instanceof FOPC.TNode) ||
					 !((FOPC.TNode)res)._bValue)) {
					...
					int sz3 = ((FOPC.ConnNode)res)._alSubNodes.size();
					boolean contains_pred = false;
					for (int pos3 = 0; pos3 < sz3; pos3++) {
					    FOPC.PNode p3 = (FOPC.PNode)
						((FOPC.ConnNode)res)._alSubNodes.get(pos3);
					    if (p3._nArity == arity && // A quick hack!
						(""+p3._sPredName).equals(""+predname)) {
						contains_pred = true;
						if (p3._bIsNegated) {
						    if (!Contains(cl._alNegClauses, res)) {
							cl._alNegClauses.add(res);
						    }
						} else {
						    if (!Contains(cl._alPosClauses, res)) {
							cl._alPosClauses.add(res);
						    }
						}
					    }					
					}
					// Only add to return set if it does not contain current
					...
					if (!((FOPC.TNode)res)._bValue) {
					    return_set.add(res);
					    if (!ALL_PROOFS) {
						return return_set; // Can exit early if only need one proof
					    }
					} else {
					    // A true TNode, don't add it (does not refute anything!)
					    //System.out.println("Tautology in resolution: " + res);
					    //System.exit(1);
					}
				    }
				    num_res++;
				...
				if (num_res >= MAX_RES_PER_ELIM) {
				    if (PRINT_MAX_RES) {
					System.out.println("...MAX_RES reached, continuing with next " + 
							   "elimination step...");
				    }
				    return return_set;
				}
    ...
    //      A(... t3_s ...)         [Note: substitution s is local!]
    //
    public ArrayList demodClauses(FOPC.ConnNode c1, int pos1, FOPC.ConnNode c2, int pos2) {

	if (PRINT_DEMOD) {
	    System.out.println("Attempting demod: " + c1.toFOLString() + ", " + c2.toFOLString());
	}
	...
	FOPC.PNode eq;
	FOPC.PNode cl_pnode;
	FOPC.ConnNode clause;
	int pnode_pos;

	if (p1._nPredID == FOPC.PNode.EQUALS && !p1._bIsNegated && c1._alSubNodes.size() == 1) {
	    eq = p1;
	    cl_pnode = p2;
	    clause = c2;
	    pnode_pos = pos2;
	...
	for (int iter = 0; iter < 2; iter++) {
	    FOPC.Term unify_term = (FOPC.Term)eq._alTermBinding.get(iter);
	    FOPC.Term subst_term = (FOPC.Term)eq._alTermBinding.get(1-iter);

	    // Check each term
	    for (int i=0; i < cl_pnode._alTermBinding.size(); i++) {
		FOPC.Term unify_term2 = (FOPC.Term)cl_pnode._alTermBinding.get(i);
		if (unify_term2 instanceof FOPC.TVar) {
		    continue;
		}

		HashMap theta = FOPC.unify(unify_term, unify_term2);
		if (theta != null) {

		    FOPC.Term to_sub = subst_term.substitute(theta);
		    FOPC.ConnNode new_clause = (FOPC.ConnNode)clause.copy();
		    FOPC.PNode new_pnode = (FOPC.PNode)new_clause._alSubNodes.get(pnode_pos);
		    new_pnode._alTermBinding.set(i, to_sub);

		    // Too strong of a rule - much simpler, see above.
		    //
		    // Search for unify_term2 in clause and replace with
		    // subst_term with substitutions made...
		    //int subs_made = 0;
		    //FOPC.Term to_sub = subst_term.substitute(theta);
		    //FOPC.ConnNode new_clause = (FOPC.ConnNode)clause.copy();
		    //Iterator j = new_clause._alSubNodes.iterator();
		    //while (j.hasNext()) {
		    //	FOPC.PNode pn = (FOPC.PNode)j.next();
		    //
		    //	for (int k = 0; k < pn._alTermBinding.size(); k++) {
		    //	    FOPC.Term tn = (FOPC.Term)pn._alTermBinding.get(k);
		    //	    if (tn.equals(unify_term2)) {
		    //		pn._alTermBinding.set(k, to_sub);
		    //		subs_made++;
		    //	    }
		    //	}
		    //}
		    ...
		    //if (subs_made < 1) {
		    //	System.out.println("ERROR: subs_made < 1");
    ...
    public FOPC.Node resolveClauses(FOPC.ConnNode c1, int pos1, FOPC.ConnNode c2, int pos2) {
	...
	// Always standardize apart clauses before unification (reset var count first)
	...
	// Ensure that we don't need to standardize apart (i.e., shared variables)
	...
	//System.out.println("Before sub: " + c1.toFOLString() + ", " + c2.toFOLString());
	c1.substitute(theta);
	c2.substitute(theta);
	//System.out.println("After sub: " + c1.toFOLString() + ", " + c2.toFOLString());
	...
	if (c1._alSubNodes.size() == 1 && c2._alSubNodes.size() == 1) {
	    ...
	    /////////////////////////////////////////////////////////////////////////
	    if (_bs != null && ((FOPC.PNode)c1._alSubNodes.get(0))._sPredName == QUERY) {

		// Add bindings to binding set if we are querying for free vars
		Iterator ans_it  = ((FOPC.PNode)c1._alSubNodes.get(0))._alTermBinding.iterator();
		...
		if (COLLECT_ONLY_CONSTANTS) {
		    ...
		    // Reset iterator
		    ans_it  = ((FOPC.PNode)c1._alSubNodes.get(0))._alTermBinding.iterator();
		}
		...
		if (collect) {
			...
			FOPC.Term ans_term = (FOPC.Term)ans_it.next();
			_bs.addBinding(entry, bind_var.toString(), ans_term.toString());
		    }
		}
	    }
	    /////////////////////////////////////////////////////////////////////////
	...
	HashSet already_added = new HashSet(); // Check for duplicates and remove!
	...
	// Note: this can be much more efficient... nodes which disjoin to prove TRUE
	...
	// ERROR Check
	//if (ret._alSubNodes.size() != (c1._alSubNodes.size() + c2._alSubNodes.size() - 2)) {
	...
	for (int j = 0; j < pnodes.size(); j++) {
		...
		FOPC.PNode pkc = null;
		if ( ((pj._sPredName == null && pk._sPredName == null && 
		       pj._nPredID == pk._nPredID) ||
		      (pj._sPredName != null && pj._sPredName.equals(""+pk._sPredName)) )
		    && pj._nArity == pk._nArity
		    && pj._bIsNegated == !pk._bIsNegated) {

		    // Generate standardized apart nodes (not sure if we have to!)
		    if (pjc == null) {
		    ...
		    //  neg_match_found = pj._alTermBinding.get(m).equals(pk._alTermBinding.get(m));
		    //}
		}
	    }
	}
	...
	if (n instanceof FOPC.PNode) {
	    ...
	    System.out.println("All clauses should be FOPC.ConnNode");
	    Object o = null; o.toString();
	    ...
	    FOPC.ConnNode cn = (FOPC.ConnNode)n;
	    FOPC.PNode p1 = (FOPC.PNode)cn._alSubNodes.get(0);
	    if (cn._alSubNodes.size() == 1 && 
		p1._nPredID == FOPC.PNode.EQUALS &&
		!p1._bIsNegated) {

		if (p1._alTermBinding.get(0) instanceof FOPC.TVar &&
		    p1._alTermBinding.get(1) instanceof FOPC.TVar) {
		    
		    System.out.println("Cannot include clause: " + n.toFOLString() + ", has free var");

		} else {
		    ClauseList cl = getClauseList(hm, "EQUALS", 0, true);
		    cl._alPosClauses.add(n);
		} 

	    } else {

		Iterator i = cn._alSubNodes.iterator();
		while (i.hasNext()) {
	...
	for (int i = 0; i < l.size(); i++) {
	    ...
	    String cur_str = p._sPredName + "_" + p._nArity + "_" + (p._bIsNegated ? "T" : "F") + k;
	    if (_m.containsKey(cur_str)) {
		cur_str = p._sPredName + "_" + p._nArity + "_" + (p._bIsNegated ? "T" : "F") + ++k;
	    }
	    _m.put(cur_str, p);
	}
	l.clear();
    ...
    public ClauseList getClauseList(HashMap hm, String predname, int arity, boolean create) {
	ClauseList cl = null;
	...
	ResetTimer();
	DemodClauseKb kb = new DemodClauseKb();
	...
	System.out.println("Build Kb  (" + GetElapsedTime() + " ms)");
	ResetTimer();
	System.out.println("RefuteKb = " + kb.canRefute() + " [ SHOULD BE YES ] (" + 
			   GetElapsedTime() + " ms)");
    }
	...
	kb.addFOPCFormula("!E ?x A(?x) ^ (B(?x) | C(?x)) ^ (D(?x) | E(?x)) ^ (F(?x) | G(?x))");
	kb.addFOPCFormula("!E ?x A(?x) | (B(?x) ^ C(?x)) | (D(?x) ^ E(?x)) | (F(?x) ^ G(?x))");
	kb.addFOPCFormula("!A ?x A(?x) ^ (B(?x) | C(?x)) ^ (D(?x) | E(?x)) ^ (F(?x) | G(?x))");
	kb.addFOPCFormula("!A ?x A(?x) | (B(?x) ^ C(?x)) | (D(?x) ^ E(?x)) | (F(?x) ^ G(?x))");
	...
	kb.addFOPCFormula("A(?x) ^ (B(?x) | C(?x)) ^ (D(?x) | E(?x)) ^ (F(?x) | G(?x))");
