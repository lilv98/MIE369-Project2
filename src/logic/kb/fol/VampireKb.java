...
public class VampireKb
	...
	// public static final String VAMPIRE_EXE = "vampire_win";
	public static final String VAMPIRE_EXE = "/Users/josephtang/Desktop/MIE369/kb-inference-main/vampire_win.exe";
    ...
    public VampireKb(float time_out) {
    	this(false, time_out);
    }
    
    // Note, no longer use versions 7/8
    ...
    //System.out.println("Vampire: setting query file to '" + f + "'");
	_queryFile = f;
    }
    ...
    public boolean runVampireQuery(String file) {
	    ...
	    Process p = Runtime.getRuntime().exec(VAMPIRE_EXE + /*(_bUse7 ? "7" : "8") +*/ " -t " + _fSeconds);
	    ...
	    String line = null;
	    BufferedReader fis_reader = new BufferedReader(new FileReader(file)); 
		...
		if (line.indexOf("Refutation found") >= 0) {
		    ref_found = true;
		} else if ((index = line.indexOf("Time elapsed:")) >= 0) {
		    _fQueryTime = getNextNum(line, index);
		} else if ((index = line.indexOf("Generated clauses:")) >= 0) {
	...
	if (CONVERT_CNF) {
	    ...
	    System.out.print((query.size() + _sClauses.size()) + " clauses.");
	} else {
	    //System.out.print("f");
	...
	try {
		...
		int axiom_ID = (Integer)_hmClause2ID.get(c);
		String vampire_clause = c.toVampireString();
		if (vampire_clause.matches(".*(\\d\\.\\d).*")) {
			os.println("% Clause CONTAINS ILLEGAL CONTENT (REAL VALUE?)\n% input_clause(clause_" + clause_cnt++ + "_axiom_" + axiom_ID + 
					",axiom,\n%   " + vampire_clause + ").\n");
			System.out.println("WARNING: cannot write axiom:\n" + vampire_clause + "\n... contains illegal content (real number?)");
		} else		
		...
		String vampire_clause = c.toVampireString();
		if (vampire_clause.matches(".*(\\d\\.\\d).*")) {
			os.println("% Clause CONTAINS ILLEGAL CONTENT (REAL VALUE?)\n% input_clause(clause_" + clause_cnt++ + ",conjecture,\n%   " + vampire_clause + ").\n");
			System.out.println("WARNING: cannot write axiom:\n" + vampire_clause + "\n... contains illegal content (real number?)");
		} else		
